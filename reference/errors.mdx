...
```
<toolName> blocked because the target range contains formula cells (<N>): <cell list>.
...
Potential spill would overwrite occupied cells (<N>): <cell list>.
...
```
Delete blocked: sheet "<title>" contains <N> rows of data.
...
| `No grid with id: <N>` | `sheetId` number no longer exists |
