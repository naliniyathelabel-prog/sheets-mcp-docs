...
**Warning:** All write tools run [safety checks](/concepts/safety) before executing. Writes that would destroy formula cells or overlap Protected Ranges are **blocked by default**. Pass `allowFormulaOverwrite: true` only when intentionally replacing formulas with static values.
